...
def mean_filter(img, filter_size=3):
    ...
    pad_width = filter_size // 2
    smoothed_img = smoothed_img[pad_width:-pad_width,
                   pad_width:-pad_width]
    return smoothed_img
...
def psnr(gt_img, smooth_img):
    ...
    if mse == 0:
        return float('inf')
    max_pixel = 255.0
    psnr_score = 20 * math.log10(max_pixel
                                 /math.sqrt(mse))
    return psnr_score
...
if __name__ == '__main__':
    ...
    img_gt = "ex1_images/ori_image.png"  # <- need to specify the path to the gt image
